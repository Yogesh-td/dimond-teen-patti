...
export class Rule_Joker {
    ...
    private Convert_Jokers(cards: Card[]): Card[] {
        const total_jokers_in_cards = cards.filter(x => x.c_number == 15).length;
        ...
        if (total_jokers_in_cards == 3) {
            ...
            const non_joker_card: Card = cards.find(x => x.c_number != 15);
            for (var i = 0; i < cards.length; i++) {
            ...
            const joker_player_card_index = cards.findIndex(x => x.c_number == 15);
