...
export class Rule_Muflis {
    ...
    Generate_Player_Scoring(players: RoundPlayerStates[]): [RoundPlayerStates[], number, CARD_HAND_TYPE] {
        ...
        var current_lowest_score: number = Number.POSITIVE_INFINITY;
        ...
        players.forEach(player => {
            ...
            if (player_score == current_lowest_score)
                winners.push(player);
            else if (player_score < current_lowest_score) {
                current_lowest_score = player_score;
        ...
        return [winners, current_lowest_score, current_hand_type];
    }
}