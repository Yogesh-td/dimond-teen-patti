...
export class Rule_Hukam
{
    ...
    Generate_Player_Scoring(players: RoundPlayerStates[], joker_card: Card): [RoundPlayerStates[], number, CARD_HAND_TYPE]{
        ...
        players.forEach(player => {
            const convertedCards = this.Convert_Jokers(Array.from(player.cards), joker_card);
    ...
    private Convert_Jokers(cards: Card[], joker: Card): Card[]
    {
        const total_jokers_in_cards = cards.filter(x => x.c_number == joker.c_number).length;
        ...
        if (total_jokers_in_cards == 3) {
            ...
            const non_joker_card: Card = cards.find(x => x.c_number != joker.c_number);
            for (var i = 0; i < cards.length; i++) {
        ...
        {
            const joker_player_card_index = cards.findIndex(x => x.c_number == joker.c_number);
